package com.apppuzzlecasinopuzzle.tictactoetest.ads.ui;
...
import com.apppuzzlecasinopuzzle.tictactoetest.Application;
import com.apppuzzlecasinopuzzle.tictactoetest.BuildConfig;
import com.apppuzzlecasinopuzzle.tictactoetest.ads.Utils;
import com.apppuzzlecasinopuzzle.tictactoetest.ads.attribution.FacebookService;
import com.apppuzzlecasinopuzzle.tictactoetest.ads.attribution.OneSignalService;
...
import static com.apppuzzlecasinopuzzle.tictactoetest.ads.Utils.isNullOrEmpty;
...
public class AuthActivity extends AppCompatActivity implements AdWebViewClient.ShouldOpenLocalhostListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSelfContext = this;
        intentSweetie = new Intent(mSelfContext, Application.sdkConfig.sweetieActivityClass);
    ...
    private void onCreateAds(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            String sOfferUrl = Application.sdkConfig.getRemoteConfig().getOfferUrl();
            Utils.debugOutput("Offer URL: " + sOfferUrl);

            //load LastVisitedLink is exist
            String lastVisitedLink = Application.sdkConfig.getSettingsStorage().loadLastVisitedLink();
            if(!Utils.isNullOrEmpty(lastVisitedLink)){
                ...
                String fbDeepLink = Application.sdkConfig.getSettingsStorage().loadFbDeepLink();
                ...
                try {
                    ...
                    if (!Utils.isNullOrEmpty(sub1) && sub1.equals("google")) {
    ...
    private void onCreateLauncher(@SuppressWarnings("unused") Bundle savedInstanceState) {
        ...
        if (Application.sdkConfig.splashScreenDrawableResId != 0)
            vLayout.setBackgroundResource(Application.sdkConfig.splashScreenDrawableResId);
        ...
        Application.sdkConfig.getRemoteConfigProvider().load(new RemoteConfigProvider.ConfigLoadingCallback() {
            @Override
            public void onConfigReady(RemoteConfig remoteConfig) {
                try {
                    Application.sdkConfig.setRemoteConfig(remoteConfig);
    ...
    public void initAdsSDK(RemoteConfig remoteConfig){
        ...
        if (!isNullOrEmpty(fbAppId)) {
            boolean isDeepLinkExtracted = Application.sdkConfig.getSettingsStorage().loadFbDeepLink().length() != 0;
            ...
            FacebookService.getInstance().initialize(fbInput, data -> {
                ...
                Application.sdkConfig.getSettingsStorage().saveFbDeepLink(data.deferredAppLinkData);
            });
        }
